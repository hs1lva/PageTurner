...
namespace PageTurnerTests.Controllers
{
    public class TrocaControllerTests
    {
        ...
        [Test]
        public async Task ProcuraMatch_ReturnsMatchingEstantes()
        {
            ...
            {
                // Arrange
                var troca = new Troca(); // Criar uma instância de Troca para acessar o método ProcuraMatch
                var minhaEstanteId = 1; // Defina o ID da sua estante de teste

                // Act
                var actionResult = await troca.ProcuraMatch(minhaEstanteId, context); // Chamar o método ProcuraMatch

                // Assert
                Assert.That(actionResult, Is.InstanceOf<ActionResult<List<Estante>>>()); // Verificar se o resultado é uma lista de Estantes

                var okResult = actionResult.Result as OkObjectResult;
                Assert.That(okResult, Is.Not.Null, "okResult não deve ser null");

                var matchingEstantes = okResult.Value as List<Estante>;
                Assert.That(matchingEstantes, Is.Not.Null, "matchingEstantes não deve ser null");

                Assert.That(matchingEstantes.Count, Is.GreaterThan(0), "Nenhuma estante encontrada");

                foreach (var estante in matchingEstantes)
                {
                    ...
                    Console.WriteLine($"Livro com match: {livro.tituloLivro}");
                    //Console.WriteLine($"Livro para troca: {estante.livro.tituloLivro}");
                    Console.WriteLine($"Usuário da outra estante: {utilizador.nome}");
                }
            }
